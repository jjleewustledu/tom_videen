	...
	char           *buf;
{
	...
	case 2:				 /* Vax I*2 */
		/* CONSTCOND */
		if (__BYTE_ORDER == __BIG_ENDIAN)
			swap_byte (buf, 512 * nblks);
		break;
	case 3:				 /* Vax I*4 */
		/* CONSTCOND */
		if (__BYTE_ORDER == __BIG_ENDIAN) {
			swap_byte (buf, 512 * nblks);
			swap_word (sbuf, 256 * nblks);
		}
		break;
	case 4:				 /* Vax R*4 */
		/* CONSTCOND */
	...
	case 7:				 /* 68K I*4 */
		break;
	default:				 /* default is to treat as Vax I*2 */
		/* CONSTCOND */
		if (__BYTE_ORDER == __BIG_ENDIAN)
			swap_byte (buf, 512 * nblks);
		break;
...
int             write_matrix_data (fptr, strtblk, nblks, buf, dtype)
	FILE           *fptr;
	int             strtblk, nblks, dtype;
	char           *buf;
{
	...
	case 1:				 /* byte format...no translation necessary */
		...
		break;
	case 2:				 /* Vax I*2 */
		stat = mat_write_idata (fptr, strtblk, buf, 512 * nblks);
		break;
	case 4:				 /* Vax R*4 */
		fbuf = (float *) buf;
		stat = mat_write_fdata (fptr, strtblk, fbuf, 256 * nblks);
		break;
	case 5:				 /* IEEE R*4 */
		stat = mat_wblk (fptr, strtblk, buf, nblks);
		break;
