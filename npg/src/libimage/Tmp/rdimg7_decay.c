...
/*  $Log */
/* =============================================================================
 *	Function:		rdimg7_decay
 *  Date:           Aug 2008
 *  Author:         Tom Videen
 *	Description:	Get image data from Version 7 file without altering decay correction
 * =============================================================================
 */
/*@unused@*/
#ifndef lint
	...
	Index          *indx;
{
	...
	if (dtype != PetVolume) {
		(void) fprintf (stderr, "ERROR [rdimg7_decay]: Unsupported file type %d for %s\n", dtype, file->name);
		return (NULL);
	}
	if (file->fp == NULL) {
		(void) fprintf (stderr, "ERROR [rdimg7_decay]: Could not get file pointer for %s\n", file->name);
		return (NULL);
	}
	if ((matnum = numcode7 (indx->frm, pln1, indx->gate, indx->data, indx->bed)) == 0) {
		(void) fprintf (stderr, "ERROR [rdimg7_decay]: Could not get matrix number for %s\n", file->name);
		return (NULL);
	}
	if ((get_sh7 (file, indx)) == ERROR) {
		(void) fprintf (stderr, "ERROR [rdimg7_decay]: Could not get subheader for %s\n", file->name);
		return (NULL);
	}
	mh7 = file->m7->mh;
	sh7 = file->m7->sh;
	...
	if ((begrec = getmatpos (file, matnum, &endrec)) == ERROR) {
		(void) fprintf (stderr, "ERROR [rdimg7_decay]: Matrix #%x not found\n", (unsigned) matnum);
		return (NULL);
	}
	if (indx->pln > 0) {
		len = xdim * ydim;
	...
	if (simg == NULL) {
		(void) fprintf (stderr, "ERROR [rdimg7_decay]: calloc simg\n");
		return (NULL);
	}
	if (fread (simg, sizeof (short), len, file->fp) != len) {
		(void) fprintf (stderr, "ERROR [rdimg7_decay]: During read\n");
		free (simg);
		return (NULL);
	}
	/* CONSTCOND  */
	...
	if (matrix == NULL) {
		(void) fprintf (stderr, "ERROR [rdimg7_decay]: Could not calloc memory for matrix\n");
		free (simg);
		return (NULL);
	}
