...
float        ***readtensor_decay (file, indx)
	ImageFile      *file;
	Index          *indx;
{
	float        ***image;
	...
	if ((image = f3tensor (1, dim3, 1, dim2, 1, dim1)) == NULL) {
		fprintf (stderr, "ERROR [readtensor_decay]: Cannot malloc image for %s\n", fname);
		return (NULL);
	}
	for (pln = 1; pln <= dim3; pln++) {
		indx->pln = pln;
		if ((img = rdimg_decay (file, indx)) == NULL) {
			fprintf (stderr, "ERROR [readtensor_decay]: Cannot read frame %d plane %d of %s\n",
