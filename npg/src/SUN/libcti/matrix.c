	...
	char           *dptr;
{
	...
	(void) mat_rblk (fptr, strtblk, dptr, nblks);
	switch (dtype) {
...
/*********************************************************/
Mat_dir	mat_read_dir (fptr, selector)
	FILE           *fptr;
	...
	Main_header    *mhead;
{
	...
	swaw (bufr, bufr, 256);
	swap_byte (bufr, bufr, 512);
	(void) mat_wblk (fptr, MatFirstDirBlk, bufr, 1);
	free (bufr);
	return (fptr);
}
	...
	int             matnum, nblks;
{
	...
	(void) mat_rblk (fptr, dirblk, dirbufr, 1);
		...
		for (i = 4; i < 128; i += 4) {
			if (dirbufr[i] == 0) {
				...
				if (oldsize < nblks) {
					...
					swaw (dirbufr, dirbufr, 256);
					swap_byte (dirbufr, dirbufr, 512);
					(void) mat_wblk (fptr, dirblk, dirbufr, 1);
					swap_byte (dirbufr, dirbufr, 512);
					swaw (dirbufr, dirbufr, 256);
	...
	unsigned char  *data;
{
	...
	for (i = 0; i < nblks; i++) {
		...
		swap_byte (bufr, bufr, 512);
		(void) mat_wblk (fptr, blk + i, bufr, 1);
		data += nbytes;
		size -= nbytes;
	}
	return 0;
