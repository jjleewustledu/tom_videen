/*$Id: import_3dscan.c,v 1.3 1995/01/30 15:58:39 ty7777 Exp $*/
/*$Log: import_3dscan.c,v $
 * Revision 1.3  1995/01/30  15:58:39  ty7777
...
static char rcsid [] = "$Header: /export/home/npggw/tom/src/recon3d/RCS/import_3dscan.c,v 1.3 1995/01/30 15:58:39 ty7777 Exp $";
  ...
  char **argv;
{
	...
	for (rd=0; rd<=rdmax; rd++)
	{
	  for (a=0; a+rd<nrings; a++)
	  {
		...
		norm_scale = compute_eff_norm( fnorm, nviews, nprojs, eff_data, a, b, dets_per_ring, input_mash);
		nv = nviews/mash;
		np = trim;
		...
		{
			fscan[i] = s1->scale_factor*p1[i]*fnorm[i]*norm_scale;
		}
		for (i=0; i<np*nv; i++)
		  fmash[i] =  0.0;
		for (i=0; i<nviews; i++)
		...
		{
		  p2 = (short int*) s2->data_ptr;
		  norm_scale = compute_eff_norm( fnorm, nviews, nprojs, eff_data, b, a, dets_per_ring, input_mash);
		  for (i=0; i<nprojs*nviews; i++)
		  {
			fscan[i] = s2->scale_factor*p2[i]*fnorm[i]*norm_scale;
		  }
		  for (i=0; i<np*nv; i++)
		    fmash[i] =  0.0;
		  for (i=0; i<nviews; i++)
