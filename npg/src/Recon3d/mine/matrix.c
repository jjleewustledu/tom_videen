/*$Id: matrix.c,v 1.2 1995/01/30 17:05:47 ty7777 Exp $*/
/*$Log: matrix.c,v $
 * Revision 1.2  1995/01/30  17:05:47  ty7777
 * Added program version.
 **/

/* @(#)matrix.c	1.8 8/24/92 */

#include	"matrix.h"

static char sccsid[]="@(#)matrix.c	1.8 8/24/92 Copyright 1989 CTI, Inc.";
static char rcsid [] = "$Header: /export/home/npggw/tom/src/recon3d/RCS/matrix.c,v 1.2 1995/01/30 17:05:47 ty7777 Exp $";
...
{
	return ((frame&0x1FF)|((bed&0xF)<<12)|((plane&0xFF)<<16)|(((plane&0x300)>>8)<<9)|
	  ...
	  FILE *fptr; int blk, nblks; short bufr[];
{
	...
	mat_rblk( fptr, blk, bufr, nblks);
	swap_byte (bufr, bufr, MatBLKSIZE*nblks);
	return (0);
}
	/*********************************************************/
  ...
  int data_size;
{
	...
	err = mat_write_image_subheader( fptr, nxtblk, header);
	if (err) return err;
	return mat_write_data( fptr, nxtblk+1, data_size, data, header->data_type);
}
  ...
  int data_size;
{
	...
	size = (data_size+511)/512;
	nxtblk = mat_enter( fptr, matnum, size);
	err = mat_write_scan_subheader( fptr, nxtblk, header);
	if (err) return err;
	return mat_write_data( fptr, nxtblk+1, data_size, data, header->data_type);
  ...
  int data_size;
{
	...
	size = (data_size+511)/512;
	nxtblk = mat_enter( fptr, matnum, size);
	err = mat_write_attn_subheader( fptr, nxtblk, header);
	if (err) return err;
	return mat_write_data( fptr, nxtblk+1, data_size, data, header->data_type);
}
  ...
  int data_size;
{
	...
	size = (data_size+511)/512;
	nxtblk = mat_enter( fptr, matnum, size);
	err = mat_write_norm_subheader( fptr, nxtblk, header);
	if (err) return err;
	return mat_write_data( fptr, nxtblk+1, data_size, data, header->data_type);
}

mat_write_data( fptr, blk, nbytes, data, dtype)
  FILE *fptr;
...
mat_read_data( fptr, blk, nbytes, data, dtype)
  FILE *fptr;
  int blk, nbytes, dtype;
  caddr_t data;
{
  ...
  unsigned char *data;
{
	...
	for (i=0; i<nblks; i++)
	{
	  ...
	  swap_byte (bufr, bufr, 512);
	  if (mat_wblk( fptr, blk+i, bufr, 1)) return -1;
	  data += nbytes;
	  size -= nbytes;
	}
	return 0;
