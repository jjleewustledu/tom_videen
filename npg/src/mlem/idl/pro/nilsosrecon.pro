...
;      true value.
;      If multiple subsets are used in a second call to NIlsosrecon, it is
...
;      NIlsosrecon for the same image, the procedure needs the previously
...
function NIlsosrecon, sino, variance, nrsubsets = nrsubsets, recon = recon, $
                   ...
                   onlysubset = onlysubset, pcoeff = pcoeff, $
                   relaxation=relaxation, backmlem=backmlem
...
if n_elements(variance) eq 0 then variance = sino > (max(sino)/1000.)
...
if n_elements(recon) le 1 $
  then recon      = fltarr(sinosize(1), sinosize(1), nplanes)
if n_elements(deltaangle) eq 0 then deltaangle = acos(-1) / nrangles

; Allocate memory for calculated sino
;---------------------------------
iter = 0
...
if n_elements(attenuation) gt 1 and n_elements(backatten) le 1 $
  ...
  backatten = recon * 0
  NIproj, backatten, attenuation^2/variance, startangle, deltaangle, $
    angles, /bil, /back, pcoeff=pcoeff
  print, 'done.'
endif
...
if n_elements(spectatten) gt 1  and n_elements(backatten) le 1 $
  ...
  print, 'Precomputing backatten...'
  backatten = recon * 0
  NIproj, backatten, 1./variance, startangle, deltaangle, nrangles, $
         /bil, /back, spectatten=spectatten, pcoeff=pcoeff
  print, 'done.'
endif

if n_elements(backatten) le 1 $
  AND NOT keyword_set(norm_per_subset) then begin
  print, 'Precomputing backatten...'
  backatten = recon * 0
  NIproj, backatten, 1./variance, startangle, deltaangle, nrangles, $
...
for i = 1,nriter do begin
  ...
  for s = 0, nrsubsets - 1 do begin
    ...
    if (onlysubset eq -1) or (s eq onlysubset) then begin
      NIlsrecon, recon, sino, variance, startangle, deltaangle, likeli = lik, $
            pcoeff = pcoeff, calc = calcsino, subset = subset, $
            attenuation = attenuation, spectatten=spectatten, $
            backatten = backatten, relaxation=relaxation, backmlem=backmlem
      lik = lik / maxlik
      if iter eq 0 and likelihood(0) eq -1 $
                        then likelihood = [lik] $
                        else likelihood = [likelihood, lik]
      if keyword_set(showsub) then NIimage, recon(*,*,nplanes / 2)
      iter = iter + 1
    endif
  endfor
  if keyword_set(showfinal) then NIimage, recon(*,*,nplanes / 2)
endfor
return, recon
end
