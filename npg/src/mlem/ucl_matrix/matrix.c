...
char           *datasettype[NumDataSetTypes] =
{
	...
	"3D Normalization", "3D Float Sinogram"
};
	...
	struct MatDir  *entry;
{
	...
	blk = MatFirstDirBlk;
	while (1) {
		read_matrix_data (fptr, blk, 1, (char *) dirbufr, SunLong);
		nfree = dirbufr[0];
		nxtblk = dirbufr[1];
		prvblk = dirbufr[2];
		nused = dirbufr[3];
...
int             mat_read_matrix_data (fptr, mhptr, blk, nblks, bufr)
	FILE           *fptr;
	Main_header    *mhptr;
	int             blk, nblks;
	short          *bufr;
	...
	Scan3D_subheader *header;
{
	...
	mat_rblk (fptr, blknum, buf, 2);
	return unmap_Scan3D_header (buf, header);
}

int             mat_read_scan_subheader (fptr, mhptr, blknum, header)
	FILE           *fptr;
	Main_header    *mhptr;
	int             blknum;
	Scan_subheader *header;
...
Mat_dir         mat_read_dir (fptr, mhptr, selector)
	FILE           *fptr;
	Main_header    *mhptr;
	char           *selector;
{
...
int             mat_enter (fptr, mhptr, matnum, nblks)
	FILE           *fptr;
	Main_header    *mhptr;
	int             matnum, nblks;
{
...
int             mat_write_image_subheader (fptr, mhptr, blknum, header)
	FILE           *fptr;
	Main_header    *mhptr;
	int             blknum;
	Image_subheader *header;
...
int             mat_write_scan_subheader (fptr, mhptr, blknum, header)
	FILE           *fptr;
	Main_header    *mhptr;
	int             blknum;
	Scan_subheader *header;
...
int             mat_write_attn_subheader (fptr, mhptr, blknum, header)
	FILE           *fptr;
	Main_header    *mhptr;
	int             blknum;
	Attn_subheader *header;
...
int             mat_write_norm_subheader (fptr, mhptr, blknum, header)
	FILE           *fptr;
	Main_header    *mhptr;
	int             blknum;
	Norm_subheader *header;
...
int             mat_read_attn_subheader (fptr, mhptr, blknum, header)
	FILE           *fptr;
	Main_header    *mhptr;
	int             blknum;
	Attn_subheader *header;
...
int             mat_read_norm_subheader (fptr, mhptr, blknum, header)
	FILE           *fptr;
	Main_header    *mhptr;
	int             blknum;
	Norm_subheader *header;
