...
Main_header_64 *h_64;
{
    ...
    h->calibration_factor = h_64->calibration_factor*ecfconverter[units];
    ...
    /* The ECAT 931 does not place a \0 after the isotope. We look for the
     * first blank and replace it with a \0.
     * J. Nuyts, 27-oct-1997
     ***********---------*/
    ptr = h_64->isotope_code;
    while ((*ptr != ' ') && (*ptr != '\0')) ptr++;
    *ptr = '\0';
    /*------ end of change --------*/
    ...
    while (j < NumberOfIsotopes && strcmp (h_64->isotope_code, isotope_info[j].name) != 0) j++;
...
Main_header* mh;
{
    ...
    h->corrections_applied = h->corrections_applied | (h_64->smoothing << 16);
    h->num_z_elements = 1;
...
Main_header* mh;
{
    ...
    h->z_pixel_size = mh->plane_separation;
    h->frame_duration = h_64->frame_duration;
    h->frame_start_time = h_64->frame_start_time;
