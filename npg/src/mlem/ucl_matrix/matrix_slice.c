...
/* static char sccsid[] = "@(#)matrix_slice.c	1.4 UCL-TOPO 97/05/06"; */
  /*
    Authors: Merence Sibomana, C. Michel, UCL.
    
    Modification: adaptation to support float PetVolume files.
                  J Nuyts, jan 1998
                  image_sub->z_dimension NOT set to 1
                  J Nuyts, feb 1998
   */
...
int	 slice, segment;
{
	...
	*data = *volume;
	...
	case PetVolume:
		...
		file_pos = matdir.strtblk*MatBLKSIZE;
                /* 2 lines added to support float PetVolume.. (J.N. 1998)
                ***************************************/
		if (data->data_type == ByteData)
                  data_size = npixels;
                else if (data->data_type == IeeeFloat)  /* ..here */
                  data_size = npixels*sizeof(float);    /* and here */
		else
                  data_size = npixels*sizeof(short);
		file_pos += slice*data_size;
		nblks = (data_size+(MatBLKSIZE-1))/MatBLKSIZE;
