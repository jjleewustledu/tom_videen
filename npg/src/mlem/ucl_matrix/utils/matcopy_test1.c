...
static 
copy_scan(mptr1, matnum, mptr2, o_matnum, storage_order)
  ...
  int             matnum, o_matnum, storage_order;
{
  ...
  case Short3dSinogram:
    ...
    if (storage_order >= 0)
      o_sh->storage_order = storage_order;
    if (o_sh->storage_order != sh->storage_order)
      keep_order = 0;
    line_size = sh->num_r_elements * sizeof(short);
  ...
  case AttenCor:
    ...
    if (storage_order >= 0)
      o_ah->storage_order = storage_order;
    if (o_ah->storage_order != ah->storage_order)
      keep_order = 0;
  ...
  char          **argv;
{
  ...
  while ((c = getopt(argc, argv, "i:o:V:s:v")) != EOF)
  {
    ...
    case 's':
	  ...
	  (storage_order != 0 && storage_order != 1))
	usage();
      break;
    case 'v':
      verbose = 1;
  ...
  if (mptr1->dirlist->nmats == 0)
    crash("no matrix in %s\n", fname);
  matnums = (int *) calloc(sizeof(int), mptr1->dirlist->nmats);
  node = mptr1->dirlist->first;
  nmats = 0;
  if (specs[0] == 0)
  {				/* no matrix specified, use first */
    matnums[nmats++] = node->matnum;
  }
  else
  {				/* build specified matnums */
    while (node != NULL)
    {
      matnum = node->matnum;
      node = node->next;
      mat_numdoc(matnum, &mat);
      if (specs[0] >= 0 && specs[0] != mat.frame)
	continue;
      if (specs[1] >= 0 && specs[1] != mat.plane)
	continue;
      if (specs[2] >= 0 && specs[2] != mat.gate)
	continue;
      if (specs[3] >= 0 && specs[3] != mat.data)
	continue;
      if (specs[4] >= 0 && specs[4] != mat.bed)
	continue;
  ...
  for (i = 0; i < nmats; i++)
  {
    ...
    else
    {
      ...
      if (matrix != NULL)
	matrix_write(mptr2, o_matnum, matrix);
    }
  }
  matrix_close(mptr1);
  matrix_close(mptr2);
