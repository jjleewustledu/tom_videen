/* ===========================================================================
 * ==== Module:			img2analyze.c Date:
 * -94 Author:			Tom Videen Description:	Transform PETT VI or
 * ECAT images into Analyze format. Input images may be any format
 * recognizable by getrealimg. Output images will be displayed by Analyze
 * with left brain on the left and with the lower slices first.  This allows
 * the 3D volume rendered brains to appear upright. History: 29-Nov-94 (TOV)
 * Create an Interfile Format header file. 21-Jul-95 (TOV) Add "atlas name"
 * and "atlas origin" to ifh output. 07-Nov-95  modified by
 * sibomana@topo.ucl.ac.be for ECAT 7 support non standard keywords start
 * with ";%" module name changed to cti2analyze 09-Jul-96 modified  by
 * sibomana@topo.ucl.ac.be : zero voxels below a threshold argument
 * ===========================================================================
 * ==== */
...
{
  fprintf(stderr,
	  "Usage:\tcti2analyze [-t data_type] [-T threshold]  [-c calibration_factor] [-p] -i PET_img -o ANALYZE_img [-h spm]\n");
  fprintf(stderr, "\tdata_type: 2=byte,4=short integer, 16=float (default is short integer)\n");
  fprintf(stderr, "\t-T threshold: thrshold is expressed as percent of image max, Voxels where value<threshold are set to 0\n");
  ...
  char           *argv[];
{
  ...
  char           *p, *ANALYZE_dir, *ANALYZE_base;
  char           *ptr;
  char            tmp[80];
  char            study_date[12];
  ...
  int             threshold = 0, threshold_val = 0;
  int             bitpix = 16;
  u_char         *b_line = NULL;
  short          *s_line = NULL;
  int             spm_header = 0;
  ...
  if ((p = strrchr(ANALYZE_base, '.')) != NULL)
    *p = '\0';

  sprintf(fname, "%s/%s.img", ANALYZE_dir, ANALYZE_base);
  if (verbose)
    fprintf(stderr, "analyze data file : %s\n", fname);
  ...
  if (matrix == NULL)
    crash("matrix %s not found\n", PET_img);

  if (calibration_factor != 1.0)
    matrix->scale_factor *= calibration_factor;
  xdim = matrix->xdim;
  ydim = matrix->ydim;
  ...
  if (matrix->data_type != SunShort && matrix->data_type != VAX_Ix2)
  {
    ...
    fprintf(stderr, "assume SunShort\n");
  ...
  memcpy(&tm, localtime(&file->mhptr->scan_start_time), sizeof(tm));
  sprintf(study_date, "%-d:%-d:%-d", tm.tm_year + 1900, tm.tm_mon, tm.tm_mday);
  ...
  case 2:
    ...
    bitpix = 8;
    if (threshold_val < 0)
      threshold_val = 0;	/* keep positive only */
    if (global_max > 255)
    {
  ...
  for (i = num_slices - 1; i >= 0; i--)
  {
	...
	if (ntohs(1) != 1)
	{
	  swab(f_line, buf_line, xdim * 4);
	  swaw(buf_line, f_line, xdim * 2);
	}
  ...
  hdr.dime.pixdim[1] = 10. * pixel_size;	/* should be input for
						 * scanner  */
  hdr.dime.pixdim[2] = 10. * pixel_size;	/* should be input for
						 * scanner  */
  hdr.dime.pixdim[3] = 10. * plane_separation;	/* z dimension of atlas   */
  if (verbose)
    fprintf(stderr, "voxel size : [%g,%g,%g] mm\n",
  ...
  else
  {
    ...
    if (ntohs(1) != 1)
      swab(spm_origin, hdr.hist.originator, 10);
    else
      memcpy(hdr.hist.originator, spm_origin, 10);
  }
