...
float f_min, f_max;
{
	...
	d_min = f_min/matrix->scale_factor;
	d_max = f_max/matrix->scale_factor;
...
static usage() {
	fprintf(stderr,
		"usage : byte_volume -i input_file,frame -o output_file -r range_min,range_max\n");
		exit(1);
}
...

{
	...
	if (i_matnum == 0) fprintf(stderr, " no frame specified\n",err++);
	if (o_fname[0] == '\0')
