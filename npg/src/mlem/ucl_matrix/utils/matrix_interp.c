...
static usage() {
	fprintf(stderr,
		"usage: matinterp -i matspec -o matspec [-V version -v]\n");
  ...
  char **argv;
{
	...
	int plane,  i, npixels, slice_blks, slice_matnum;
	int trimmed=0, elem_size =2, offset = 0;
	short *sdata;
	u_char *bdata;
	struct Matval mat;
	extern char *optarg;

	while ((c = getopt (argc, argv, "i:o:V:v")) != EOF) {
		switch (c) {
		case 'i' :
			in_spec	= optarg;
            break;
	...
	for (plane=1; plane<=matrix->zdim; plane++) {
		...
		trimmed = matrix_trim(slice,100,100);
		matrix_float(slice);
		...
		slice_matnum = mat_numcod(1,plane,1,0,0);
