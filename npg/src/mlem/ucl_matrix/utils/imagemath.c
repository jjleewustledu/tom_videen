...
char *pgm;
{
	fprintf(stderr,"usage: %s imagea imageb imagec c1,c2,c3 op1,op2\n%s%s",pgm,
		 "\twhere op1={+,-,*,/,not,and} and op2={log,exp,sqr}\n",
		 "\timagec=op2(c1+(c2*imagea)op1(c3*imageb))\n");
...
/* get imagea */
	...
	image1 = matrix_read(file1,matnum,MAT_SUB_HEADER);
	...
	if (file2) {
		image2 = matrix_read(file2,matnum,MAT_SUB_HEADER);
	...
	for (plane=0; plane< image1->zdim; plane++) {
		...
		switch( op1)
		{
	  	...
	  	case '/':
			...
			break;
		case 'a' :		/* c1*imagea if (imageb!=0) */
			for (i=0; i<npixels; i++)
			imagea[i] = (imageb[i]==0.0)? 0.0 : c2*imagea[i];
			break;
		case 'n' :	/* c1*imagea if (imageb==0) */
			for (i=0; i<npixels; i++)
			imagea[i] = (imageb[i]==0.0)? c2*imagea[i] : 0.0;
			break;
	  	default:
			crash("%s: illegal operator \"%c\"...chose from {+,-,*,/,and,not}\n",
			argv[0], op1);
		}
		switch (op2) {
		case 'l' :
