...
static float find_data_max(data) 
MatrixData *data;
{
...
static flip_x(line,data_type,xdim)
caddr_t line;
int data_type,xdim;
{
...
static flip_y(plane,data_type,xdim,ydim)
caddr_t plane;
...
char *str;
{
	...
	if (sscanf(p,"%d %d %d",&r,&g,&b) == 3) { /*valid entry */
		cc += strlen(p);
	...
	while (!end_of_table && fgets(line,82,fp) != NULL) {
		...
		if (sscanf(line,"%d %d %d",&r,&g,&b) == 3) { /*valid entry */
			cc += strlen(line);
...
#endif
{
    ...
    if (_is_interfile(hdr_fname) && (fp = fopen(hdr_fname, "r"))!=NULL) {
...
char **ifh;
{
	...
	if (ifh[NUMBER_FORMAT] && strstr(ifh[NUMBER_FORMAT], "float")) {
		if (elem_size != 4)
		return interfile_error( "unsupported number of pixels for float type\n");
		if (big_endian) imagesub->data_type = IeeeFloat;
		else imagesub->data_type = VAX_Rx4;
	} else {  /* integer data type */
		if (elem_size != 1 && elem_size != 2 & elem_size != 4)
		return interfile_error("unsupported number of pixels for int type\n");
		...
		if (big_endian) {
			...
			if (elem_size == 2) imagesub->data_type = VAX_Ix2;
			if (elem_size == 4) imagesub->data_type = VAX_Ix4;
		}
	}
...
int   matnum, dtype;
{
	...
	if (ifh[ATLAS_ORIGIN_1] &&
		...
		if (x_flip) data->x_origin = (data->xdim-f-1)* data->pixel_size;
		else data->x_origin = f * data->pixel_size;
	}
	if (ifh[ATLAS_ORIGIN_2] &&
		sscanf(ifh[ATLAS_ORIGIN_2], "%g", &f) == 1) {
		if (y_flip) data->y_origin = (data->ydim-f-1) * data->y_size;
		else data->y_origin = f * data->y_size;
	}
	if (ifh[ATLAS_ORIGIN_3] &&
		sscanf(ifh[ATLAS_ORIGIN_3], "%g", &f) == 1) {
		if (z_flip) data->z_origin = (data->zdim-f-1) * data->z_size;
		else data->z_origin = f  * data->z_size;
	}
	return OK;
}
