/*
 general information: 
 ML-EM reconstruction of PET emission studies, including normalization and
 ...
 After reconstruction, the module zoom_matrix is called to zoom and shift
 the image, using exactly the same conventions as those in the reconstruction
 software of CTI-Siemens, for the Ecat 931.
 */
/******************************************************************************
  ...
  if (!copy)
    ...
    for (y_pos = 0; y_pos < dim_out_y; y_pos++)
      ...
      for (x_pos = 0; x_pos < dim_out_x; x_pos++)
         ...
         * Otherwise, use the routine unzoom_2d, which adds pixels
...
float*   read_norm_sinogram(
  ...
  if (is3dsino)
    {
    norm_fl = NCreadnormv7(sinogram_name, normalisation_name, 
                           plane, frame, gate, bed, verbose, 1, 1, 1);
    return(norm_fl);
    }
  else
    {
    ...
    if (verbose) printf("Mean normalization value = %g\n", meanval);
    return(norm_fl);
    }  
  }
  ...
  if (*sensit == NULL)
    ...
    for (proj = 0; proj < nrproj; proj++)
      ...
      for (det = 0; det < newdet; det++)
        ...
        if (*p_smallsensit < 0) *p_smallsensit = 0.0; 
        p_smallsensit++;
        p_smallsino++;
        }
      }
...

{
  ...
  *******************************/
  ...
  if (!sinogram_file)
    {
    if (verbose) printf("Not a 2D sinogram, opening as 3D sinogram...\n");
  ...
  ********************/
  if (is3dsino)
    {
    /* The 2D sinogram is stored in a 3D file format. It is read by opening
     * the entire volume with matrix_read, and then reading the desired slice
     * with matrix_read_slice. The segment is set to 0, since only the
     * 2D data are supposed to be avaible.
     */
   ...
   * Create a prototype mainheader to do so.
   * If no version is specified, the same version and datatype as
   * that of the sinogram are used.
  ...
  if (version >= V7) proto.file_type = PetVolume;
  if (verbose)
    {
    printf("Write in software version %d, datatype ", version);
    if (data_type == VAX_I2)    printf("VAX_I2\n");
    if (data_type == SUN_I2)    printf("Sun short int\n");
    if (data_type == IeeeFloat) printf("Sun float\n");
    }

  /* Opening or creating the output file
  *******************************/
  if (verbose) printf("Opening %s as PetImage...\n", image_name);
  image_file = matrix_open(image_name, MAT_READ_WRITE, PetImage);
  if (!image_file)
    {
    if (verbose) printf("Opening %s as PetVolume...\n", image_name);
  ...
  *********************************/
  ...
  /* Allocate memory for sensitivity matrix. Sensitivity is computed as
   * 1. / attenuation_correction / normalisation_correction.
   * By using this approach, the statistics will be closer to Poisson than
   * when precorrection is applied.
  *****************************************/
  if (attn || norm)
    {
    ...
    for (i = 0; i < nrdet*nrproj; i++)
      {
      *++p_sensit = 1.0;
      if (attn) *p_sensit /= *p_atten++;
      if (norm) *p_sensit /= *p_norm++;
      }
    if (attn) free(atten_fl);
    if (norm) free(norm_fl);
    }

  /* Apply sum-detectors: adding adjecent detectors to reduce sinogram size
   * and hence computation time.
   *****************************/       
  if (sumdet > 1)       
    {
    if (verbose) printf("Summing sinogram %d neighbor pixels\n", sumdet);
  ...
  *****************************/
  ...
  if (lsrelaxation == 0.0)
    ...
    **************************************/
    if (verbose) printf("Starting reconstruction, cpu = %g\n", cpu_s());
    ...
    if (verbose) printf("End of reconstruction, cpu = %g\n", cpu_s());
    ...
    **************************************/
    ...
    for (iter = 0; iter < n_iter; iter++)
      ...
      for (mainiter = 0; mainiter < iteraties[iter]; mainiter++)
        {
        /* Starting with regular ML is more robust (prevents streaks which
         * only disappear after sufficient iterations).
         *****/
        if (iternum == 0)
          relaxation = 0.0;
        else
    ...
    if (sensit != NULL) free(sensit);
    if (backmlem != NULL) free(backmlem);
    free(sinovar);
    }
  ...
  *****************/
  ...
  if (data_type != IeeeFloat)
    {
    /* Compute the scale factor and convert float to short int, if
     * that is required by the data_type.
    *********************************************************/
    ...
    for (i = 0; i < size_image * size_image; i++)
      {
      if (*++p_image_zoom < image_min) image_min = *p_image_zoom;
      if (  *p_image_zoom > image_max) image_max = *p_image_zoom;
      }
    if (image_max < fabs(image_min)) image_max = fabs(image_min);
    if (image_max != 0.0)
  ...
  /* The same counts are reconstructed in a smaller number of
   * pixels, so the pixel values have been raised by sumdet*sumdet.
  ****************/
  sumdet_factor = 1. / ((float) (sumdet * sumdet));

  scale_factor = decay_factor * sumdet_factor / cscale 
                      / (frame_duration / 1000.0);

  /* For float files, scale the whole image. Otherwise, put the scalefactor
   * in the header.
   ************/
  if ((data_type == IeeeFloat))
    {
  ...
  ***************************/
  ...
  img_sub->processing_code         = norm + 2*attn +
                      256*(arc_radius != 0.0) + 512*decay;
  ...
  NCmatrix_write_plane(image_file, imagematrix, 
                       outplane, outframe, outgate, outbed);
}
