/* module NCproj_attback */

/*
 general information: This module computes the attenuated
 backprojection of an image along parallel projection lines. It uses
  ...
  for (det = 0; det < prcf->bigN; det++)
    ...
    if (prcf->linefirst1[det] >= 0)
      for (i = prcf->linefirst1[det]; i < firstcommon; i++)
        ...
        mu     += selfmu;
        *(imgplusoffset + *i1++) += proj * *w1++ * exp(-mu);
        mu     += selfmu;
        }
    ...
    if (prcf->linefirst2[det] >= 0)
      for (i = prcf->linefirst2[det]; i < firstcommon; i++)
        ...
        mu     += selfmu;
        *(imgplusoffset + *i2++) += proj * *w2++ * exp(-mu);
        mu     += selfmu;
        }
    ...
    for (i = firstcommon; i < lastcommon; i++)
      ...
      mu     += selfmu;
      expminmu = exp(-mu);
      *(imgplusoffset + *i1++) += proj * *w1++ * expminmu;
      *(imgplusoffset + *i2++) += proj * *w2++ * expminmu;
      mu     += selfmu;
      }
