...
void NCprojcoeff_first_and_last(
  ...
  *last  = -2;
  ...
  while (pix < N && *first == -1)
    ...
    if ((curcol >= 0) && (curcol < N) &&
        (currow >= 0) && (currow < N) && (distance2 <= radius2))
      {
      *first = pix;
      }
    pix++;
...
p_ProjCoeff NCprojcoeff(
  ...
  for (projnum = 0; projnum < nrproj; projnum++)
    ...
    for (i = 0; i < prcf->N; i++)
      ...
      prcf->bigweight1[i] = (prcf->bigindex2[i] - bigcoord) / scale;
      prcf->bigweight2[i] = (bigcoord - prcf->bigindex1[i]) / scale;
    ...
    for (i = 0; i < prcf->bigN; i++)
      ...
      prcf->smallweight1[i] = prcf->smallindex2[i] - smallcoord;
      prcf->smallweight2[i] = smallcoord - prcf->smallindex1[i];
