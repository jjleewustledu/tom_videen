/*$Id: sspct.h,v 1.1 1996/06/10 08:57:20 jane Exp $*/
/*$Log: sspct.h,v $
...
typedef enum {
    ...
    INSTITUTION,                /* where the scan was done (Mallinckrodt Institute) */
...
typedef struct {
    ...
    char  table_incr[10];	/* table increase */
    ...
    char  comment1[25];         /* "H/SP CONTRAST", for instance -- typed by technologist? */
    char  comment2[25];         /* "PET SCAN STUDY", for instance -- typed by technologist */
} SCAN_INFO;	/* scan info. -- same for all slices in a set */

typedef struct {
    char  filename[50];		/* name of file containing data for particular slice */
    ...
    char  unknown[9];           /* usually "TH 7502" for scouts, "AH 7572" otherwise */
...
extern void get_sspct_head(FILE *fp, SSPCT_INIT hdrinit, SSPCT_HDR *head_info, DSR *anal_hdr, FILE *fp_if);
