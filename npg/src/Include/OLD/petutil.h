/*$Id: petutil.h,v 1.8 1995/12/01 15:06:18 tom Exp tom $*/
/*$Log: petutil.h,v $
...
#ifndef lint
static char     rcsid_petutil[] = "$Header: /home/npggw/tom/include/RCS/petutil.h,v 1.8 1995/12/01 15:06:18 tom Exp tom $";
#endif
...
#define	ZATLAS_DIMENSION		7.0	 	/* vertical dimension from center of AC-PC in atlas */
...
#define	ECAT_OFFSET				352.8	 /* default ECAT offset from reference slice in mm */
#define	ECAT_OFFSET_AFTER_OCT94	351.7	 /* After Oct, 94, ECAT offset from reference slice in  mm*/
...
typedef struct {
	...
	char            run_name[6];		 /* specify the tracer name: OC1, OC2, OO1, OO2 */
...
typedef struct {
	...
	float           dap; 			 /* distance from the A-P center of the reference
						  			  ...
						  			  * the center of the AC-PC and the reference slice (cm) */
...
typedef struct {
	...
	POINT2D         right;
	POINT2D         top;
}               PET_SLICE_DATA;		 /* structure for a single PET slice, used in xray */
...
}               COMMENT_INFO;		    /* Possible information contained in field
						  				 * study_description of the ECAT image main header */
...
void     DrawSymbol ();
int      Ecat2Pett6 ();
void     FillHeaderPett6 ();
int      GetECATBasicHeader ();
int      GetImagePett6 ();
int      GetImageSuperPett ();
int      GetPetParameters ();
int      GetVersion ();
void     lowercase ();
void     PutImagePett6 ();
void     Transformation ();
