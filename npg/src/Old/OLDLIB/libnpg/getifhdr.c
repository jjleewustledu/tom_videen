/*$Id: getifhdr.c,v 1.6 1995/11/10 16:28:21 tom Exp $*/
/*$Log: getifhdr.c,v $
 * Revision 1.6  1995/11/10  16:28:21  tom
 * remove petutil parent directory from include statements
 *
 * Revision 1.5  1995/11/03  22:37:50  ty7777
 * Added first study and last study.
 *
...
static char rcsid [] = "$Header: /home/npggw/tom/src/libnpg/RCS/getifhdr.c,v 1.6 1995/11/10 16:28:21 tom Exp $";
	...
	IFH	*ifhdr;
{
	...
	for (index = 0; index < nlines; index++)
	{
		...
		if ((value = strstr (lines [index], ":=")) != NULL)
		{
			...
			if (strstr (label, "first study") != NULL)
			{
				if (DetermineType (parameter) == DATA_TYPE_INTEGER)
					ifhdr->first_study	= atoi (parameter);
				else
				{
					if (print_message)
						error_message ("first study", 
			...
			if (strstr (label, "last study") != NULL)
			{
				if (DetermineType (parameter) == DATA_TYPE_INTEGER)
					ifhdr->last_study	= atoi (parameter);
				else
				{
					if (print_message)
						error_message ("last study", 
						original_parameter, ifhdrfile);
					return FAIL;
				}
			...
			if (strstr (label, "start image id") != NULL)
			{
				if (DetermineType (parameter) == DATA_TYPE_INTEGER)
					ifhdr->start_image_id	= atoi (parameter);
				else
				{
					if (print_message)
						error_message ("start image id", 
						original_parameter, ifhdrfile);
					return FAIL;
				}
			...
			if (strstr (label, "end image id") != NULL)
			{
				if (DetermineType (parameter) == DATA_TYPE_INTEGER)
					ifhdr->end_image_id	= atoi (parameter);
				else
				{
					if (print_message)
						error_message ("end image id", 
						original_parameter, ifhdrfile);
					return FAIL;
				}
