...
C$Id: fftsun.f,v 1.2 1993/10/21 19:22:01 ty7777 Exp $
C$Log: fftsun.f,v $
c Revision 1.2  1993/10/21  19:22:01  ty7777
c Change HEADER to Header.
c
c Revision 1.1  1993/10/21  19:20:42  ty7777
c Initial revision
c*/
C
C-------------------------------------------------------------------
...
C
      DATA ISIZE(1), ISIZE(2), ISIZE(3), ISIZE(4), ISIZE(5)
     *   /1, 1, 1, 2, 2/
...
C
      ...
      RCSHEADER = '$Header: /home/petsun4/ty7777/src/libnpg/RCS/fftsun.f,v 1.2 1993/10/21 19:22:01 ty7777 Exp $'
C
...
C
      END
C--MACHALL, I1MACH + R1MACH + D1MACH  FROM IEEE
C    BY MIKIO YAMAMOTO, DEC 10,1981
...
C
C     MACHINE CONSTATNS FOR SUN AZS 30-Dec-92
C
      DATA IMACH( 1) /   5 /
      DATA IMACH( 2) /   6 /
      DATA IMACH( 3) /   7 /
...
C
C  WHERE POSSIBLE, OCTAL OR HEXADECIMAL CONSTANTS HAVE BEEN USED  !NOT AZS
...
C
C     Modified 30-Dec-92 AZS
C
      DATA RMACH(1) / '00800000'X /
      DATA RMACH(2) / '7f7fffff'x /
      DATA RMACH(3) / '3B100000'X /  !What id spacing?
      DATA RMACH(4) / '3C100000'X /
      DATA RMACH(5) / '41134413'X /
      IF (I .LT. 1  .OR.  I .GT. 5) GOTO 100
C
      R1MACH = RMACH(I)
      IF(I.EQ.5)R1MACH=ALOG10(2.)
      RETURN
C
 100  IWUNIT = I1MACH(4)
      WRITE(IWUNIT, 99)
...
C
C
C     MODIFIED 20-Dec-92 AZS
C
C      IF (I .LT. 1  .OR.  I .GT. 5) GOTO 100
C
C      IF(I.EQ.1)D1MACH = d_min_normal()  !  SUN numerical computation guide p.48
C      IF(I.EQ.2)D1MACH = d_max_normal() 
C      IF(I.EQ.3)D1MACH = d_min_subnormal()  !?
C      IF(I.EQ.4)D1MACH = d_max_subnormal()  !? 
C      IF(I.EQ.5)D1MACH = DLOG10(DBLE(2.))
C      RETURN
C
C 100  IWUNIT = I1MACH(4)
C      WRITE(IWUNIT, 99)
