...
C     input variables MINV and MAXV (MINV <= R <= MAXV).
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE RELFIELD (R, MINV, MAXV, LLEN, ROW, COL, WARNLINE, NEXT)
C
      INTEGER*4    LLEN, ROW, COL, WARNLINE, MINLEN, MAXLEN, STRLEN, 
...
C
     ...
     3          ' ***'//BELL
      CALL RELFORMT (R,LLEN,NUMSTR)
      L1	= INANYL (NUMSTR, ' 0')
      STRLEN	= INANYR (NUMSTR, ' 0')
100   STR	= NUMSTR (L1:STRLEN)
      CALL INFIELD (STR, LLEN, ROW, COL, NEXT)
C
...
C
      IF ((X .LT. MINV) .OR. (X .GT. MAXV)) GO TO 8010
C
