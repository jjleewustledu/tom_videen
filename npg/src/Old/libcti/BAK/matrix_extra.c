...
static int             matrix_close (matfile)
	MatrixFile     *matfile;
{
...
static int             read_sun_data (mfp, matnum, data)
	MatrixFile     *mfp;
	MatrixData     *data;
	int             matnum;
{
...
static int             write_sun_data (mfp, matnum, data)
	MatrixFile     *mfp;
	MatrixData     *data;
	int             matnum;
{
...
static int             read_matrix_data (fptr, strtblk, nblks, dptr, dtype)
	FILE           *fptr;
...
static struct MatDir  *matrix_find (matnum, matfile)
	int             matnum;
	MatrixFile     *matfile;
{
	MatDirNode     *node;
...
static int             insert_mdir (matdir, dirlist)
	MatDirList     *dirlist;
	struct MatDir   matdir;
{
	MatDirNode     *node;
...
static MatDirBlk      *mat_rdirblk (fptr, blknum)
	FILE           *fptr;
	int             blknum;
{
	MatDirBlk      *matdirblk;
...
static MatDirList     *mat_read_directory (fptr)
	FILE           *fptr;
{
	struct MatDir   matdir;
	MatDirList     *dirlist;
	...
	MatrixFile     *mp;
{
	...
	if (fptr == NULL)
		return (0);
	stat = mat_write_main_header (fptr, mp->mhptr);
	if (stat != 0) {
		(void) mat_close (fptr);
		free (fptr);
		return (0);
	...
	count = 0;
	do {
		stat = mat_rblk (mp->fptr, blkno, buf, 1);
		if (stat != 0)
			break;
		stat = mat_wblk (fptr, blkno, buf, 1);
		if (stat != 0)
			return (0);
