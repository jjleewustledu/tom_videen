c***********************************************************************
c
c    SUBROUTINE TITLE		pt6maskml.f
c
...
c
c	call maskml(xinc, yinc, dinc, datamaskdmin, datamaskdmax,
c    +	   backmaskymin, backmaskymax, backmaskxymin, backmaskxymax,
c    +	   backmaskdstart, forwmaskdmin, forwmaskdmax, forwmasknumt,
c    +	   forwmaskxstart, forwmaskystart)
c
c	where
c
...
c			"angle" and (d) index "dindex."  (output)
c
...
c***********************************************************************
      ...
      subroutine maskml(xinc, yinc, dinc, datamaskdmin, datamaskdmax,
     ...
     +	 baseindex, datamaskdmax, datamaskdmin, dindex, forwmaskdmax,
     +	 forwmaskdmin, forwmasknumt(dindexmin:dindexmax), index,
     +	 tindexa, tindexb, xindexa, xindexb, yindex
      real backmaskdstart(maskbackindexmin:maskbackindexmax),
     +	 costhe(angindexmin:angindexmax), d,
     +	 dinc(angindexmin:angindexmax), dindexstart, dsampvtsamp,
     +	 forwmaskxstart(maskforwindexmin:maskforwindexmax),
     +	 forwmaskystart(maskforwindexmin:maskforwindexmax), pixvdsamp,
     +	 pixvtsamp, radback, radbacksq, radforw, radforwsq, radtest,
     +	 sinthe(angindexmin:angindexmax), ta, tb, theta, tinit,
     +	 tsampvpix, x, xa, xb, xinc(angindexmin:angindexmax),
     +	 xindexstart, xinit, y, yinc(angindexmin:angindexmax),
     +	 yindexstart
...
c    Compute sampling ratios.
	 ...
	 pixvdsamp   = pix   / dsamp
	 pixvtsamp   = pix   / tsamp
	 dsampvtsamp = dsamp / tsamp

c    Compute the increment tables.
c    xinc is the increment in the x index that results from an increment
c	by unity in the pseudo-t index.
c    yinc is the increment in the y index that results from an increment
c	by unity in the pseudo-t index.
...
c	by unity in the x index.
	 ...
	 do 1000 angle = angindexmin, angindexmax
	    ...
	    xinc(angle) = -sinthe(angle) * tsampvpix
	    yinc(angle) = -costhe(angle) * tsampvpix
...
c    projection mask.
	 ...
	 radforw = (radback * pixvtsamp) + 1.0
	 radforwsq = radforw * radforw

	 do 3000 dindex = forwmaskdmin, forwmaskdmax

	    d = (float(dindex) - dindexcent) * dsampvtsamp
	    tb = sqrt(radforwsq - (d * d))
	    ta = -tb
	    tindexb = int(tb + tindexcent + 1.0)
	    tindexa = int(ta + tindexcent)
	    forwmasknumt(dindex) = tindexb - tindexa + 1
	    tinit = float(tindexa) - tindexcent - 1.0

	    do 3005 angle = angindexmin, angindexmax

	       index = (angle * numd) + dindex
	       x = ((d * costhe(angle)) - (tinit * sinthe(angle)))
     +		  * tsampvpix
	       y = ((-tinit * costhe(angle)) - (d * sinthe(angle)))
     +		  * tsampvpix
	       xindexstart = x + xindexcent
	       yindexstart = y + yindexcent
	       forwmaskxstart(index) = xindexstart
	       forwmaskystart(index) = yindexstart
