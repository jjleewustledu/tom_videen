...
char ** argv;
{
    ...
    if ( strcmp ( argv[progarg], "mlmany" ) == 0  )
    {
        /* the idea behind "mlmany" is that a small shell script */
        /* will run in the background and issue one */
        /* mlmany rootname all all 12     command after another */
    ...
    {
        fprintf(stderr,"driver: Fatal Name Error    %s\n",argv[progarg]);
        fprintf(stderr,"Legal names are  mlmany, ml, em, and fbp\n");
        exit (-1);
    }
    if ( argc < argcmin )
    {
    ...
    if ( emflag )
    {
        ...
        for ( i=1; i <= Nslices; i++ )
        {
            fprintf(shout,"%s/pt6gensurvprob %s %s .survprob%d %d >>& logfile\n",
                ecompath, norm, trans, i, i);
        }
    }
    fflush ( shout );
    ...
    for ( icomp = 0; icomp < ncmpnd; icomp++ )
    {
        ...
        {  /* start of EM-specific script generation */
            ...
            for ( j = 0; j < emiscnt; j++ )
            {
                ...
                for ( i = 1; i <= Nslices; i++ )
                {
                    fprintf(shout,"%s/pt6intfloat %s .emission %d >>& logfile\n", ecompath,
                        emissions[k],i);
                    fprintf(shout,"%s/pt6intfloat %s .randoms %d >>& logfile\n", ecompath,
                        randoms[k],i);
                ...
                for ( l = 0; l < rescount; l++ )
                {
                    ...
                    fprintf(shout,
                        ...
                        ecompath, mip, MIRSCALE, outname, ext1, resext[l], ext3 );
    ...
    for ( j = 0; j < emiscnt; j++ )
    {
        ...
        for ( i = 1; i <= Nslices; i++ )
        {
            fprintf(shout,"%s/%s %s %s %s %s .pcoremis%d %d >>& logfile\n",
                ecompath, precor, norm, trans, emissions[k], randoms[k], i, i);
        }
        ...
        for ( l = 0; l < rescount; l++ )
        {
            for ( i = 1; i <= Nslices; i++ )
            {
                fprintf(shout,"%s/%s .pcoremis%d .fbpim%d %s >>& logfile\n",
                    ecompath, fbp, i, i, argv[l+argcmin-1]);
            }
            fprintf(shout,
