...
/*$Id: xray.c,v 1.12 1995/11/27 20:40:49 ty7777 Exp tom $*/
/*$Log: xray.c,v $
...
static char     rcsid[] = "$Header: /home/petsun19/tom/src/xray/RCS/xray.c,v 1.12 1995/11/27 20:40:49 ty7777 Exp tom $";
 ...
 */
	...
	if (pettnum == ERROR_SCANNER) {
		fprintf (stderr, "Unrecognized scanner type\n");
		exit (FAIL);
	}
	close (imgfd);
...
/*
 * strcpy (postscript_file, sxr_root); strcat (postscript_file, ".ps");
 * print_data.draw_data		= &draw_data; print_data.cursor		= define_cursor;
 * print_data.drawing_points	= &drawing_points; print_data.cols			=
 * cols; print_data.rows			= rows; print_data.xray_tracing		=
 * xray_tracing; print_data.postscript_file	= postscript_file; XtAddCallback
 * (cascadeSaveWinDump, XmNactivateCallback, PrintCallback, &print_data);
 */
