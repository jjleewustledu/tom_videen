...
C	AND WRITES VALUES FOR 4 AXIAL FOVS
C
C  Intent:
...
C
     ...
     &    USEROUT,HDRONLY,ERR)
        IF (ERR) THEN
          ...
          STOP
        END IF
        CALL GETIMG(MASK,HEADER,MASKNAME,SLICE,NSLICES,PETTNUM,INFILE,
     &    USEROUT,HDRONLY,ERR)
        IF (ERR) THEN
        ...
        DO 500 J = 1,NDIM
          DO 400 I = 1,NDIM
            IF (MASK(I,J) .GE. 1) THEN
              Sum  = 0.5 * (Scale1 * FLOAT(IMAGE1(K,L))) - (Scale2 * FLOAT(IMAGE2(K,L)))
              Diff = (Scale1 * FLOAT(IMAGE1(K,L))) + (Scale2 * FLOAT(IMAGE2(K,L)))
              SumMean  = SumMean + Sum
              SumDiff  = SumDiff + Diff
              SumSqr   = SumSqr  + Diff**2
							NREG = NREG + 1
            END IF
400       CONTINUE
500     CONTINUE
...
C
			DO 1200 I=1,4
				...
				DO 1100 SLICE = S1(I), S2(I)
					Sum = Sum + SliceSED(SLICE)/SliceMean(SLICE)
					NSLICES = NSLICES + 1
1100		CONTINUE
...
C  Write results
C
			WRITE(USEROUT,8600) (FOV(I), I=1,4)
			WRITE(USEROUT,8600) (FOVSED(I), I=1,4)
C
8600  FORMAT(4(F10.4))
C
      STOP
