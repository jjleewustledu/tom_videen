 ...
 */
	...
	fixed = matrix (1, nfid, 1, MM);
	reslice = matrix (1, nfid, 1, MM);
 ...
 */
	...
	mat = matrix (1, MM, 1, MM);
	aligned = vector (1, MM);
	matcreate (mat, p);

	printf ("\n\nTransformation matrix:\n");
	matrix_print (mat, MM, MM);
	...
	printf ("     X           Y           Z          error\n");
	for (i = 1; i <= nfid; i++) {
		matrix_mult (mat, reslice[i], aligned, MM, MM, 1);
	...
	free_matrix (fixed, 1, nfid, 1, MM);
	free_matrix (reslice, 1, nfid, 1, MM);
	free_matrix (x, 1, NPRM, 1, NPRM);
	free_vector (p, 1, NPRM);
	free_matrix (mat, 1, MM, 1, MM);
	free_vector (aligned, 1, MM);
}
