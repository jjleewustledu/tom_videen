; $Id: jpg2if.pro,v 1.2 2005/03/01 21:53:40 tom Exp $
;+
; NAME:
;   JPG2IF
;
; PURPOSE:
;   Convert JPG to Interfile 
;
; CATEGORY:
;   Image processing
;
; CALLING SEQUENCE:
;   JPG2IF Infile Outfile
;
...
@IMAGE_UTILS.h
   ...
   ;; Reorientation assumes film was scanned so that JPG images
   ...
   FOR j=first,last DO BEGIN
      ...
      tmp = read_tiff(infile)
      ;img = 0.3*tmp[0,*,*]+0.3*tmp[1,*,*]+0.3*tmp[2,*,*]
      ;FOR i=0,xdim-1 DO image[i,*,j-first] = fix(img[0,x1+2*i,y1:y2])
      img = rebin(tmp,1,xdim,ydim)
      FOR i=0,xdim-1 DO image[i,*,j-first] = fix(img[0,i,*])
   ENDFOR
   ...
   printf, lu1, 'conversion program := jpg2if.pro'
   printf, lu1, 'program date       := 2008:01:02'
