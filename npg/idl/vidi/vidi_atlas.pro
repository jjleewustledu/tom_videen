PRO VIDI_ATLAS, uv, idx, temp
   ;; Display Talairach atlas outline on the image in window idx
   ...
   IF (atlas[idx] AND dsp[idx].set) THEN BEGIN
      ...
      IF (ap GE 0) THEN BEGIN
         ...
         IF ((x0 GE 0) AND (y0 GE 0) AND (x1 LT xsize) AND (y1 LT ysize)) THEN BEGIN
            tmp = extrac(temp,x0,y2,adim[0],adim[1])
