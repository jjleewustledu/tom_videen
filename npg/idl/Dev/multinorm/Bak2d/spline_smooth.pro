...
;               [ XTITLE= ,YTITLE=, INTERP=, /SILENT,/PLOT,/ERRBAR]
;
; INPUT PARAMETERS:
...
;       described in "Fundamentals of Digital Image Processing" by A. Jain
...
;       None.
; EXAMPLE:
;       Obtain coefficients of a univariate smoothing spline fitted to data X,Y
;       assuming normally distributed errors Yerr and write the results to a file.
;
;       IDL> SPLINE_SMOOTH, X, Y, Yerr, distance, coefficients, smoothness, t='spline.dat'
;
...
; REVISIONS
;       Jan 12, 1994    I. Freedman (HSTX)  Adjusted formats
;       Mar 14, 1994    I. Freedman (HSTX)  Improved convergence
;       Mar 15, 1994    I. Freedman (HSTX)  User-specified interpolates
;       Sep 1997		Converted to IDL V5.0   W. Landsman
;       Feb 1999		Call PLOTERROR instead of PLOTERR  W. Landsman
;-

PRO spline_smooth, X, Y, Yerr, distance, coefficients, smoothness, xplot, yplot, $
        XTITLE=xtitle, YTITLE=ytitle, INTERP=interp, SILENT=silent, PLOT=plot, ERRBAR=errbar
   ...
   IF N_PARAMS() LT 4 THEN BEGIN
      print,'Syntax: SPLINE_SMOOTH, X, Y, Yerr, distance, [coefficients, smoothness, xplot, yplot,'
      print,'        XTITLE=, YTITLE=, INTERP=, /SILENT, /PLOT, /ERRBAR]'
      RETURN
   ENDIF
...
; Plot
      ...
      ENDELSE
      msg = rmsg +', distance = '+string(FORMAT='(G10.4)',distance)+ $
           ', smoothness = '+string(FORMAT='(G10.4)',smoothness)
      title = strcompress(msg)
      symbol = 7                ; x symbol
