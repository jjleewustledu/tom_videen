...
if aheader.scale lt 1d-30 then aheader.scale = 0
...
if total(tal_orig  - [-92, -128, -74]) eq 0 then begin
	idl_correct = 1
	tal_orig = -(aheader.orig - idl_correct)*vox_sizes
endif
mt(3,0:2) = tal_orig
...
;----------------------------------------------------
;Attempt to read any .mat file for dicom conversions
;----------------------------------------------------
if total(aheader.magnet_transform(3,*)) eq 0 then begin
	matfile = findfile(strmid(filename,0,strlen(filename) -3) + 'mat',count = matcount)
	if matcount gt 0 then begin
		matreply = read_mat(filename = matfile(0))
		print,'Reading in matfile for magnet_transform'
		;---------------------------------------
		;Use the .mat
		;The .m will have an analyze-flip in it
		;---------------------------------------
		mt = matreply.mat
		;-----------------------------------------------------
		;Correct for matlab coordinates (set matlab_correct to 1 if desired)
		;-----------------------------------------------------
		mtf = fltarr(4,4)
		for i = 0,3 do mtf(i,i) = 1
		mtf(3,*) = 1
		mtb = mtf
		mtb(3,0:2) = -1
		;-------------------------------
		;individual steps
		;magnet2matlab=invert(mt)
		;magnet2idl=(magnet2matlab#mtb)
		;idl2magnet=invert(magnet2idl)
		;mt = idl2magnet
		;-------------------------------
		if keyword_set(matlab_correct) then begin
			print,'Corrected for matlab coordinates'
			mt=invert( ( invert(mt) # mtb) )		
		endif
		landmark = matreply.landmark
		if landmark eq 0 then begin
			extract_pname,filename,p
			landmark = total(float(byte(p)))
		endif
		print,'Autogenerated landmark'	
		print,mt,landmark	
	endif	
	aheader.magnet_transform = mt
endif else if idl_correct then aheader.magnet_transform = mt
if aheader.landmark eq 0 then aheader.landmark = landmark
...
pro make_4D_analyze,filename = filename,P = P,outfilename = outfilename,remove = remove,flip=flip
;-------------------------------------
;converts list of analyze files to 4D
;set flip to right-left flip images
;-------------------------------------
...
for i = 0,n_elements(P) - 1 do begin
	...
	if keyword_set(flip) then for j = 0,ah.z - 1 do cube(*,*,j) = rotate(cube(*,*,j),5)
