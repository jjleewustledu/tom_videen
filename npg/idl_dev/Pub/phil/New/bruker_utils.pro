...
pro readreco, path, rows, cols, slices, dim, wordtype, fovx, fovy, fovz, ID=id
   a = ""
   b = ""
   rows = 0
   cols = 0
...
pro readimnd, path, slices, tr, te, navgs, slice_thick, nexpts, scantime, ID=id
...
pro readacqp, path, slices, tr, te, slice_thick, scantime, ID=id
...
function readbruker, file_path, DATA_INFO=data_info, PREVIEW=preview, ID=id
   ...
   ;; make sure that file_path is a real directory and user has executable access.
   ;; If the test is tru then result returns '0' so have to fix and not it
