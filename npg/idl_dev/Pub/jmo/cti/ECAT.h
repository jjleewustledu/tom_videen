...
;
; PURPOSE:
; ECAT.h defines structures in ECAT sinogram and image files
; and error codes used in ECAT.pro routines.
;
; DEFINED:
;	FH = main header file info
;	PI = main header patient info
;	SI = main header scan info
;	MH = main header (MH, PI, SI)
;	SH = subheader scan info
;	RH = subheader reconstruction info
;	IH = subheader image info (with RH)
;	NH = subheader norm info
;	AH = subheader atten info
;	PM = subheader       info
;	!ECAT
;
; CATEGORY:
...
;	Define System Variable !ECAT as ECAT_DATA Structure
;	---------------------------------------------------

	defsysv,'!ECAT',{ECAT_DATA,			$
				file_types:strarr(14),	$
				data_types:strarr(8),	$
