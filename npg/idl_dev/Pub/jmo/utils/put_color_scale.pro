pro put_color_scale,x1,y1,range,inc,ysize=ysize,charsize=charsize,TITLE=title
;+
; ROUTINE:    put_color_scale
;
; USEAGE:     PUT_COLOR_SCALE,x1,y1,range[,inc,ysize=ysize,charsize=charsize,TITLE=title]
;
...
if nzs lt 0 and nzs gt -4 then begin
  frmt='(f8.'+string(form='(i1)',-nzs+2)+')'  ; used on scale
endif
...
for ival=0,nval do begin
  ...
  if ss ge 0 and ss le 1 then begin
    ...
    plots,[x1,x2],[yval,yval],/device,COLOR=!WHITE_INDEX ; JMO 1/27/94
    xyouts,x3,yval,sval,/device,charsize=charsize,COLOR=!WHITE_INDEX ;JMO 1/27/94
  endif
endfor
