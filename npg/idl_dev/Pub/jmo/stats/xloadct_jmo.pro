; $Id: xloadct_jmo.pro,v 1.3 1997/01/07 22:18:12 jmo Exp jmo $
...
COMMON xloadct_com, r0, g0, b0, tfun, state, filename, cps, psave, pnt, $
	...
	gamma, color, use_values, ncolors, cbot,table_index
...
if event.id eq state.name_list then begin
	table_index = event.index
	fi.color_scale(fi.n) = table_index
	dsp.color_table_index=table_index
...
PRO XLoadct_jmo, PICK_ONE=PICK_ONE, SILENT=silent_f, GROUP = GROUP, FILE=file, $
...
;gamma = fix(50*(alog10(fi.color_gamma(fi.n)) + 1 ))
vbot = 0
vtop = 100
gamma = 1.
use_values=0
...
base = WIDGET_BASE(TITLE="XLoadct", /COLUMN,KILL_NOTIFY='xloadct_jmo_dead')
